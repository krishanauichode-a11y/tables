  ...
  try {
    ...
    if (webinarLeads && webinarLeads.length > 0) {
      webinarLeads.forEach(item => {
        ...
        if (!formattedData.webinarLeads[year]) formattedData.webinarLeads[year] = {};
    ...
    if (webinarData && webinarData.length > 0) {
      webinarData.forEach(item => {
        const year = item.year || "2026";
        if (!formattedData.webinarData[year]) formattedData.webinarData[year] = {};
    ...
    if (webinarPerformanceData && webinarPerformanceData.length > 0) {
      webinarPerformanceData.forEach(item => {
        ...
        if (empName) {
          const year = item.year || "2026";
          if (!formattedData.webinarPerformanceData[year]) formattedData.webinarPerformanceData[year] = {};
          if (!formattedData.webinarPerformanceData[year][empName]) formattedData.webinarPerformanceData[year][empName] = Array(12).fill(0);
    ...
    employees.forEach(emp => {
      ...
      empDailyWebinar.forEach(entry => {
        ...
        if (!formattedData.webinarDailyData[year]) formattedData.webinarDailyData[year] = {};
        if (!formattedData.webinarDailyData[year][emp.name]) formattedData.webinarDailyData[year][emp.name] = {};
        if (!formattedData.webinarDailyData[year][emp.name][entry.month]) formattedData.webinarDailyData[year][emp.name][entry.month] = {};
...
app.post('/api/sales', async (req, res) => {
  try {
    const { 
      employees, employeeOrder, dailyBookings, dailyBookingsByYear, leadSummary, 
      monthlyLeads, monthlyLeadsByYear, batchData, monthlyBatchAdmin, customHeaders, 
      webinarLeads, employeeBatches, batchToMonthMapping, webinarData,
      webinarPerformanceData, webinarDailyData 
    } = req.body;
      ...
      for (const empName in dailyBookings) { 
        ...
        for (const month in dailyBookings[empName]) { 
          for (const day in dailyBookings[empName][month]) { 
            dailyBookingsToUpsert.push({ employee_id: empId, month: parseInt(month), day: parseInt(day), value: dailyBookings[empName][month][day], year: "2026" }); 
          } 
        } 
      }
    }
        ...
        for (const year in dailyBookingsByYear[empName]) {
          for (const month in dailyBookingsByYear[empName][year]) {
            for (const day in dailyBookingsByYear[empName][year][month]) {
              dailyBookingsToUpsert.push({ employee_id: empId, month: parseInt(month), day: parseInt(day), value: dailyBookingsByYear[empName][year][month][day], year: year });
            }
          }
        }
      }
    ...
    if (dailyBookingsToUpsert.length > 0) await upsertData('daily_bookings', dailyBookingsToUpsert, 'employee_id, month, day, year');
    ...
    for (const empName in leadSummary) { 
      ...
      if (typeof monthlySummary === 'object' && monthlySummary !== null) {
        for (const monthKey in monthlySummary) {
          const month = parseInt(monthKey, 10); if (isNaN(month)) continue;
          const summary = monthlySummary[monthKey];
          leadSummaryToUpsert.push({ employee_id: empId, month: month, fre: summary.pre || 0, off: summary.off || 0, rep: summary.rep || 0, fam: summary.app || 0, bas: summary.bas || 0, adv: summary.adv || 0, att: summary.att || 0 });
        }
      }
    } 
    ...
    if (monthlyLeads) {
      for (const empName in monthlyLeads) { 
        const empId = empIdMap[empName]; if (!empId) continue; 
        for (let month = 0; month < 12; month++) { 
          monthlyLeadsToUpsert.push({ employee_id: empId, month: month, value: monthlyLeads[empName][month] || 0, year: "2026" }); 
        } 
      }
    }
    if (monthlyLeadsByYear) {
      for (const empName in monthlyLeadsByYear) {
        ...
        for (const year in monthlyLeadsByYear[empName]) {
          for (let month = 0; month < 12; month++) {
            monthlyLeadsToUpsert.push({ employee_id: empId, month: month, value: monthlyLeadsByYear[empName][year][month] || 0, year: year });
          }
        }
      }
    }
    await supabase.from('monthly_leads').delete().in('employee_id', employeeIds);
    if (monthlyLeadsToUpsert.length > 0) await upsertData('monthly_leads', monthlyLeadsToUpsert, 'employee_id, month, year');
    ...
    // --- Save Custom Headers ---
    if (customHeaders) {
      await supabase.from('custom_headers').delete().neq('id', 0);
      ...
      if (headersToInsert.length > 0) await supabase.from('custom_headers').insert(headersToInsert);
    }

    // --- Save Employee Order ---
    if (employeeOrder && Array.isArray(employeeOrder)) {
      await supabase.from('custom_headers').insert({ table_name: '_employeeOrder', headers: employeeOrder });
    }
    ...
    if (monthlyBatchAdmin && Object.keys(monthlyBatchAdmin).length > 0) {
      ...
      if (adminDataToInsert.length > 0) await supabase.from('monthly_batch_admin_leads').insert(adminDataToInsert);
    }
    ...
    if (webinarLeads) {
      ...
      for (const year of Object.keys(webinarLeads)) {
        await supabase.from('webinar_leads').delete().eq('year', year);
      }
      if (webinarLeadsToUpsert.length > 0) await supabase.from('webinar_leads').insert(webinarLeadsToUpsert);
    }
    ...
    if (employeeBatches) {
      ...
      if (batchAssignments.length > 0) await supabase.from('employee_batches').insert(batchAssignments);
    }
    ...
    if (batchToMonthMapping) {
      ...
      const mappingsToInsert = batchToMonthMapping.map(mapping => ({
        batch_index: mapping.batchIndex, batch_name: mapping.batchName, month_index: mapping.monthIndex,
        month_name: mapping.monthName, year: mapping.year || "2026"
      }));
      if (mappingsToInsert.length > 0) await supabase.from('batch_month_mapping').insert(mappingsToInsert);
    }
    ...
    if (webinarData) {
      ...
      for (const year in webinarData) {
        for (const batchName in webinarData[year]) {
          webinarDataToInsert.push({ year: year, batch_name: batchName, lead_count: webinarData[year][batchName] });
        }
      }
      if (webinarDataToInsert.length > 0) await supabase.from('webinar_data').insert(webinarDataToInsert);
    }
        ...
        for (const empName in webinarPerformanceData[year]) {
          ...
          if (Array.isArray(monthlyData)) {
            monthlyData.forEach((val, monthIndex) => {
              perfDataToInsert.push({ employee_id: empId, employee_name: empName, year: String(year), month: monthIndex, lead_count: Number(val) || 0 });
            });
          }
        }
      }
      if (perfDataToInsert.length > 0) await supabase.from('webinar_performance').insert(perfDataToInsert);
    }
          ...
          for (const month in webinarDailyData[year][empName]) { 
            for (const day in webinarDailyData[year][empName][month]) { 
              dailyWebinarToUpsert.push({
                employee_id: empId, year: parseInt(year), month: parseInt(month),
                day: parseInt(day), value: webinarDailyData[year][empName][month][day]
              });
            }
          }
        }
      }
      if (dailyWebinarToUpsert.length > 0) await upsertData('daily_webinar_performance', dailyWebinarToUpsert, 'employee_id, year, month, day');
    }
...
// Remove employee (FIXED VERSION)
app.delete('/api/employee/:name', async (req, res) => {
  try {
    // Decode URL encoded names (handles spaces and special characters)
    const name = decodeURIComponent(req.params.name);
    console.log(`>>> [DELETE-DEBUG] Attempting to remove employee: "${name}"`);

    if (!name || name.trim() === '') {
      return res.status(400).json({ error: 'Employee name is required' });
    }

    // 1. Find employee safely using maybeSingle()
    ...
    if (empError) {
      console.error('>>> [DELETE-DEBUG] DB error finding employee:', empError);
      return res.status(500).json({ error: 'Database error', details: empError.message });
    }

    if (!employee) {
      console.log(`>>> [DELETE-DEBUG] Employee "${name}" not found.`);
      return res.status(404).json({ error: 'Employee not found', searchedName: name });
    }

    const employeeId = employee.id;
    console.log(`>>> [DELETE-DEBUG] Found employee ID: ${employeeId}`);

    // 2. Remove from custom_headers order list
    try {
      ...
      if (orderRow && Array.isArray(orderRow.headers)) {
        const updatedOrder = orderRow.headers.filter(emp => emp !== name);
        await supabase.from('custom_headers').update({ headers: updatedOrder }).eq('id', orderRow.id);
        console.log(`>>> [DELETE-DEBUG] Removed "${name}" from employee order.`);
      }
    } catch (e) { 
      console.warn('>>> [DELETE-DEBUG] Warning: Could not update order list:', e.message); 
    }

    // 3. Remove current batch assignment
    try {
      await supabase.from('employee_batches').delete().eq('employee_id', employeeId);
      console.log(`>>> [DELETE-DEBUG] Removed batch assignment.`);
    } catch (e) { 
      console.warn('>>> [DELETE-DEBUG] Warning: Could not remove batch assignment:', e.message); 
    }
    ...
    if (deleteError) {
      console.error('>>> [DELETE-DEBUG] Delete failed:', deleteError);
      
      // If it fails with Foreign Key error (code 23503), it means the SQL below wasn't run
      if (deleteError.code === '23503') {
        return res.status(409).json({ 
          error: 'Foreign key constraint prevents deletion',
          details: 'Please run the SQL fix in your Supabase SQL Editor to allow deletion.',
    ...
    console.log(`>>> [DELETE-DEBUG] SUCCESS: Employee "${name}" deleted.`);
    res.json({ success: true, message: `Employee "${name}" removed.` });

  } catch (error) {
    console.error('!!! [DELETE-DEBUG] CATASTROPHIC ERROR !!!', error);
